 ...
 */
import { useAppConfig, useChatStore } from "@/app/store"; // Assuming these types exist
...
export function getNewStuff(
  ...
  const isNewModel =
    model.endsWith("-preview") || model.startsWith("gemini-pro");
  ...
  if (isNewModel || isDalle) {
    ...
    return {
      max_tokens: 128000,
      maxOutputTokens: 128000, // Assign the same value to maxOutputTokens
      isNewModel: true,
      payloadType,
      isDalle,
    };
