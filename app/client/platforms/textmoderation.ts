  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `[${
          response.status
        }] Failed to get moderation response: ${JSON.stringify(errorBody)}`,
      );
    }
    ...
    if (!contentType || !contentType.includes("application/json")) {
      throw new Error(
        "Unexpected content type received from moderation response",
      );
    }
    ...
    if (moderationJson.results && moderationJson.results.length > 0) {
      ...
      if (!moderationResult.flagged) {
        ...
        if (!fallbackResponse.ok) {
          ...
          console.error(
            `Fallback moderation request failed: ${JSON.stringify(errorBody)}`,
          );
          throw new Error(
            `[${
              fallbackResponse.status
            }] Failed to get fallback moderation response: ${JSON.stringify(
              errorBody,
            )}`,
          );
        }
      ...
      console.error(
        `[${provider}] [Text Moderation] Moderation response is empty`,
      );
      throw new Error(
        `[${provider}] [Text Moderation] Moderation response is empty`,
      );
...
export async function moderateText(
  ...
  try {
    ...
    if (moderationResponse.flagged) {
      const flaggedCategories = Object.entries(moderationResponse.categories)
...
function logModerationResult(
  ...
  console.log(
    `[${provider}] [Text Moderation] flagged:`,
  ...
  if (moderationResult.flagged) {
    const flaggedCategories = Object.entries(moderationResult.categories)
      ...
      .map(([category]) => category);
    console.log(
      `[${provider}] [Text Moderation] flagged categories:`,
      flaggedCategories,
    );
  }
}
...
function formatModerationMessage(flaggedCategories: string[]): string {
  const translatedReasons = flaggedCategories.map((category) => {
    const translation =
      Locale.Error.Content_Policy.Reason[
        category as keyof typeof Locale.Error.Content_Policy.Reason
      ] || category;
    return translation;
  });
